...
class TestAUCJudd:
    ...
    def test_accumulation(self):
        ...
        assert len(metric.accumulated_values[0]) == batch_size, f"Should have {batch_size} values in layer 0"
...
class TestPearsonCorrelationCoefficient:
    ...
    def test_perfect_correlation(self):
        ...
        assert abs(result.item() - 1.0) < 1e-5, f"Expected correlation = 1.0, got {result.item()}"
    ...
    def test_no_correlation(self):
        ...
        assert abs(result.item()) < 0.5, f"Expected correlation ≈ 0, got {result.item()}"
    ...
    def test_accumulation(self):
        ...
        # Check accumulation
        assert len(metric.accumulated_values) == 3, "Should have 3 layers"
        assert len(metric.accumulated_values[0]) == batch_size, f"Should have {batch_size} values"
        ...
        assert len(values) == 3, "Should have 3 layer values"
        assert all(-1 <= v <= 1 for v in values), "All correlation values should be in [-1, 1]"
    ...
    def test_multi_dimensional_squeeze(self):
        ...
        assert not torch.isnan(result) or result.item() != 0, "Should calculate without dimension errors"
