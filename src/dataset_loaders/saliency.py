...
class SaliencyMIT1003DatasetLoader(BaseSaliencyDatasetLoader):
    DATASET_PATH = MIT1003_DATASET_PATH = (
    ...
    def get_paths(self) -> dict[str, list[str]]:
        ...
        for stim_file in stimulus_files:
            ...
            if (
                base_name not in saliency_files_map
                or base_name not in fixation_files_map
            ):
                print(
                    f"[WARNING] No matching saliency/fixation for {stim_file}, skipping..."
                )
                continue

            stimulus_paths.append(os.path.join(stimuli_dir, stim_file))
            saliency_paths.append(
                os.path.join(saliency_dir, saliency_files_map[base_name])
            )
            fixation_paths.append(
                os.path.join(fixation_dir, fixation_files_map[base_name])
            )
...
class SaliencyMIT1003Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            resize_function = torchvision.transforms.Compose(
                [
                    x
                    for x in self.transform.transforms
                    if not isinstance(x, torchvision.transforms.Normalize)
                ]
            )
...
class SaliencyMIT1003TorchDatasetLoader(
    BaseTorchDatasetLoader, SaliencyMIT1003DatasetLoader
):
    def __init__(
        self,
        batch_size: int,
