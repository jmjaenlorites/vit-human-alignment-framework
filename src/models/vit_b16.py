            ...
            def create_hook(block_idx, attn_module):
                ...
                def hook_fn(module, input, output):
                    ...
                    if result is None:
                        ...
                        protect_mask = torch.zeros(N, N, device=x.device, dtype=torch.bool)
                        protect_mask[0, :] = True
                        protect_mask[:, 0] = True
                        ...
                        if self.discard_ratio > 0 and valid_entries > 0:
                            k = min(int(total_entries * self.discard_ratio), valid_entries)
                    ...
                    if k > 0:
                        ...
                        _, indices_to_drop = torch.topk(protected, k, dim=1, largest=False)
                    ...
                    if block_idx in target_layers:
                        ...
                        # Normalizar
                        masks_normalized = masks_2d / masks_2d.amax(dim=(1, 2), keepdim=True).clamp_min(self._EPS)
