...
from ..dataset_loaders.saliency import BaseSaliencyDatasetLoader, SaliencyMIT1003TorchDatasetLoader
...
class PearsonCorrelationCoefficient(BaseSaliencyMetric):
    ...
    def _calculate_torch(self, pred_map: torch.Tensor, ground_truth_saliency_map: torch.Tensor, ground_truth_fixation_map: torch.Tensor) -> torch.Tensor:
        ...
        print(f"Pearson correlation coefficient denominator: {denom} and eps: {self._EPS}")
        if denom <= self._EPS:
            return torch.tensor(float("nan"), device=pred_map.device, dtype=torch.float)
...
class SaliencyMetricsCalculator(BaseMetricCalculator):
    def __init__(self, backend: BackendEnum, metrics: Optional[list[BaseSaliencyMetric]] = None):
        super().__init__("SaliencyMetricsCalculator")
        self._metrics: list[BaseSaliencyMetric] = metrics or [AUC_Judd(backend), PearsonCorrelationCoefficient(backend)]
        self._backend = backend
        self._return_saliency = True

    def get_dataset_loader(self, transform: Optional[Callable[[Any], Any]] = None) -> BaseSaliencyDatasetLoader:
        """Carga dataset y metadatos necesarios (responsabilidad local)."""
        match self._backend:
            case BackendEnum.TORCH:
                return SaliencyMIT1003TorchDatasetLoader(batch_size=32, shuffle=False, num_workers=2, transform=transform)
