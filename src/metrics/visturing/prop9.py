"""Métrica para Propiedad 9: Frequency masking."""
...
from ...dataset_loaders.visturing.prop9 import Prop9TorchDatasetLoader
from ...utils.common_enums import BackendEnum


class FrequencyMaskingKendall(BaseVisTuringMetric):
    """
    Calcula correlación de Kendall para frequency masking.

    Evalúa el efecto de máscaras de frecuencia en la visibilidad.

    Note: When instantiated from CSV, uses default freq='low' (3 cpd).
    Future JSON configuration will allow specifying frequency and mask frequency.
    """

    name: str = "visturing_frequency_masking_kendall"
    ...
    def calculate(self, batch: Any, batch_results: Any) -> None:
        ...
        if len(batch) < 5:
            raise ValueError(
                "Prop9 requires batch metadata (c_group, mask, contrast_idx)"
            )
        c_groups = batch[2]
        masks = batch[3]
...
def create_prop9_calculator(
    backend: BackendEnum,
    freq: str = "all",  # 'low', 'high', 'all'
    mask_freq: str = "1p5",  # '1p5', '3', '6', '12', '24'
    data_path: Optional[str] = None,
    gt_path: Optional[str] = None,
    batch_size: int = 32,
) -> VisTuringCalculator:
    """
    Factory para crear un calculator de Prop9.

    Args:
        backend: Backend (torch o jax)
        freq: Frecuencia ('low' para 3cpd, 'high' para 12cpd)
        mask_freq: Frecuencia de máscara ('1p5', '3', '6', '12', '24')
        data_path: Ruta a los datos
        gt_path: Ruta al ground truth
        batch_size: Tamaño de batch

    Returns:
        VisTuringCalculator configurado para Prop9
    """
    # Mapear freq a C1/C2
    ...
    if freq == "all":
        ...
        gabor_key = f"a_{freq_map[freq]}_Fmask_{mask_freq}"

    dataset_loader = Prop9TorchDatasetLoader(
        gabor_key=gabor_key,
    ...
    # Crear métrica
    metric = FrequencyMaskingKendall(
    ...
    calculator = VisTuringCalculator(
        ...
        name=f"Prop9Calculator_{freq}_Fmask_{mask_freq}",
    )
